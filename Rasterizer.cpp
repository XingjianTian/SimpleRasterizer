...
Scanline Rasterizer::generate_scanline(Vertex vl,Vertex vr,int y)
{
	...
	scanline.dv = (vr.v-vl.v)*divide;//微分
...
{
	if (renderState==RenderState::WIREFRAME)//线框图，内无
	{
	...
	{
		draw_top_flat_triangle(v1,v2,v3);//平边三角形
	}
	...
	else//非平边
	{
		...
		Vertex v4 = v1.interp(v3,factor);//插值
