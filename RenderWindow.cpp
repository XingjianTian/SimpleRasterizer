...
RenderWindow::RenderWindow(const string name_t,const string title_t,int width,int height):name(name_t),title(title_t),WIDTH(width),HEIGHT(height)
{
	//1.创建窗口类
	WNDCLASSEX wndClass = {};
...
void RenderWindow::message_dispatch()
{
	//时间初始化
	...
	while (msg.message!=WM_QUIT)
	{
		...
		else
		{
			...
			{
				preTime = curTime;
				RenderManager::Instance().Draw_update();
			}
		}
	}
}
...
LRESULT CALLBACK RenderWindow::WndProc(HWND hwnd,UINT message,WPARAM wparam,LPARAM lParam)
{
	...
	case WM_DESTROY:
		RenderManager::Instance().Draw_end();
		PostQuitMessage(0);
		//break;
	case WM_KEYDOWN:
