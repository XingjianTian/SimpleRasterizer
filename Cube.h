...
struct Material
{
	VColor diffuse;//对漫反射光的反射率
	VColor specular;//对镜面光的反射率
	VColor ambient;//对环境光的反射率
};
...
public:
	Cube(int n,string path,Vector3 pos):local_vertexes(24,Vertex()),trans_vertexes(24,Vertex()),texture(path),position(pos)
	{
		...
		for (int i=0;i<24;++i)
		{
			...
			VColor color(0,colors[i][0],colors[i][1],colors[i][2]);//颜色
			Vertex vertex(v,color,uv[i%4][0],uv[i%4][1]);//顶点
			v.normalize();//规范化
			vertex.set_normal(v);
